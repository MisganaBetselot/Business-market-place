...
// Badge colors use Tailwind's default palette (amber/purple/green/red/gray)
// since only cream/charcoal/primary/primary-dark/primary-light/accent are
// defined as project tokens — not enough distinct colors for 5 states.
// ---------------------------------------------------------------------------
const STATUS = {
...
export default function ReceiptUpload() {
  ...
  // Same fallback pattern as PaymentInstructions.jsx: find the current
  // PENDING (or REJECTED, awaiting re-upload) subscription from the
  // backend. This is also how we get the actual subscription id to
  // attach the receipt to, since the route carries no :id param.
  const {
  ...
  const sellerSubscriptionId = fallbackSubscription?.id;
  const displayPlan = selectedPlan || fallbackSubscription?.plan;
  ...
  const currentState = statusFromReceipt(fallbackSubscription?.status, receipt);
          ...
          <>
            ...
            <div className="mt-8 flex items-start justify-between rounded-xl bg-white p-6">
              <div>
                ...
                <p className="mt-3 font-serif text-xl font-bold text-charcoal">
                  {displayPlan.media_type === "VIDEO" ? "Video" : "Photo"}{" "}
            ...
            <div className="mt-6">
              {loading ? (
                ...
                <StatusPanel state={currentState} subscription={fallbackSubscription} />
              )}

              <div className="mt-6">
                <button
                  type="button"
                  onClick={() => navigate("/sell/payment-instructions", { state: { selectedPlan: displayPlan } })}
    ...
    <div>
      {rejectionReason && (
        <div className="mb-6 flex items-start gap-3 rounded-lg bg-red-50 px-5 py-4 text-red-700">
          ...
          <div>
            <p className="font-sans font-medium">Your last receipt was rejected</p>
            <p className="mt-1 font-sans text-sm">{rejectionReason}</p>
          </div>
        </div>
      )}
      ...
      <div
        ...
        className={`flex flex-col items-center justify-center rounded-xl border-2 border-dashed px-6 py-16 text-center transition-colors ${
          isDragging ? "border-primary bg-primary-light/10" : "border-charcoal/15 bg-white/60"
        }`}
      >
        {file ? (
          <>
            ...
            <p className="mt-5 font-sans font-medium text-charcoal">{file.name}</p>
          ...
          <>
            ...
            <p className="mt-1 font-sans text-charcoal/50">or browse from your device</p>
            <button
              type="button"
      ...
      {fileError && <p className="mt-3 font-sans text-sm text-red-700">{fileError}</p>}
      {submitError && <p className="mt-3 font-sans text-sm text-red-700">{submitError}</p>}
...
// ---------------------------------------------------------------------------
function StatusPanel({ state, subscription }) {
  const config = {
    UNDER_REVIEW: {
      icon: Clock3,
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-charcoal/15 bg-white/60 px-6 py-16 text-center">
      ...
      <p className="mt-5 font-sans text-lg font-medium text-charcoal">{config.title}</p>
      <p className="mt-2 max-w-md font-sans text-charcoal/70">{config.body}</p>
    </div>
  );
}