...
export default function FavoriteListingCard({ listing, onUnsave, removing }) {
  ...
  return (
    <Link to={`/business/${listing.id}`} className="block h-full">
      <Card
        className={`group flex h-full flex-col overflow-hidden border-2 border-brand-100 p-0 transition-all duration-200 hover:-translate-y-1 hover:border-brand-500 hover:shadow-lg hover:shadow-brand-500/20 ${
          removing ? "opacity-50" : ""
        }`}
      >
